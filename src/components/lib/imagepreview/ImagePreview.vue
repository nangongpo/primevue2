<template>
    <span :class="containerClass" :style="styleName" v-bind="ptmi('root')">
        ...
        <button v-if="preview" ref="previewButton" :aria-label="zoomImageAriaLabel" type="button" :class="cx('button')" @click="onImageClick" v-bind="{ ...previewButtonProps, ...ptm('button') }">
            <slot name="indicatoricon">
                <DynamicComponent :template="indicatorIcon ? 'i' : 'EyeIcon'" :className="cx('icon')" v-bind="ptm('icon')" />
            </slot>
        </button>
        <Portal>
            <div v-if="maskVisible" :ref="maskRef" v-focustrap role="dialog" :class="cx('mask')" :aria-modal="maskVisible" @click="onMaskClick" @keydown="onMaskKeydown" v-bind="ptm('mask')">
                ...
                <transition name="p-image-preview" @before-enter="onBeforeEnter" @enter="onEnter" @leave="onLeave" @before-leave="onBeforeLeave" @after-leave="onAfterLeave" v-bind="ptm('transition')">
                    <div v-if="previewVisible" v-bind="ptm('previewContainer')">
                        <slot name="preview" :className="cx('preview')" :style="imagePreviewStyle" :onClick="onPreviewImageClick" :previewCallback="onPreviewImageClick">
...
<script>
import FocusTrap from 'primevue2/focustrap';
import EyeIcon from 'primevue2/icons/eye';
import RefreshIcon from 'primevue2/icons/refresh';
import SearchMinusIcon from 'primevue2/icons/searchminus';
import SearchPlusIcon from 'primevue2/icons/searchplus';
import TimesIcon from 'primevue2/icons/times';
import UndoIcon from 'primevue2/icons/undo';
import Portal from 'primevue2/portal';
import { DomHandler, ZIndexUtils } from 'primevue2/utils';
import BaseImagePreview from './BaseImagePreview.vue';

export default {
    name: 'ImagePreview',
    extends: BaseImagePreview,
    inheritAttrs: false,
