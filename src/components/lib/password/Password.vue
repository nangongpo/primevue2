...
import EyeSlashIcon from 'primevue2/icons/eyeslash';
...
export default {
    ...
    inheritAttrs: false,
    emits: ['input', 'change', 'focus', 'blur', 'invalid'],
    data() {
        return {
            id: this.$attrs.id,
    ...
    methods: {
        ...
        onFocus(event) {
            ...
            if (this.feedback) {
                this.setPasswordMeter(this.value);
        ...
        setPasswordMeter() {
            ...
            const { meter, label } = this.checkPasswordStrength(this.value);
