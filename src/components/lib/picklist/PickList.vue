...
import AngleDoubleDownIcon from 'primevue2/icons/angledoubledown';
import AngleDoubleLeftIcon from 'primevue2/icons/angledoubleleft';
import AngleDoubleRightIcon from 'primevue2/icons/angledoubleright';
import AngleDoubleUpIcon from 'primevue2/icons/angledoubleup';
import AngleDownIcon from 'primevue2/icons/angledown';
import AngleLeftIcon from 'primevue2/icons/angleleft';
import AngleRightIcon from 'primevue2/icons/angleright';
import AngleUpIcon from 'primevue2/icons/angleup';
...
export default {
    ...
    inheritAttrs: false,
    emits: ['input', 'reorder', 'update:selection', 'selection-change', 'move-to-target', 'move-to-source', 'move-all-to-target', 'move-all-to-source', 'focus', 'blur'],
    itemTouched: false,
    ...
    methods: {
        ...
        onEnterKey(event, listType) {
            ...
            this.onItemClick(event, this.value[listId][matchedOptionIndex], matchedOptionIndex, listId);
        ...
        onSpaceKey(event, listType) {
            ...
            if (event.shiftKey && this.d_selection && this.d_selection.length > 0) {
                const listId = listType === 'sourceList' ? 0 : 1;
                const selectedItemIndex = ObjectUtils.findIndexInList(this.d_selection[listId][0], [...this.value[listId]]);
                const matchedOptionIndex = this.findMatchedOptionIndex(listType);

                this.d_selection[listId] = [...this.value[listId]].slice(Math.min(selectedItemIndex, matchedOptionIndex), Math.max(selectedItemIndex, matchedOptionIndex) + 1);
                this.$emit('update:selection', this.d_selection);
                this.$emit('selection-change', {
                    originalEvent: event,
                    value: this.d_selection
        ...
        onHomeKey(event, listType) {
            if (event.ctrlKey && event.shiftKey) {
                ...
                this.d_selection[listId] = [...this.value[listId]].slice(0, matchedOptionIndex + 1);
                this.$emit('update:selection', this.d_selection);
                this.$emit('selection-change', {
                    originalEvent: event,
                    value: this.d_selection
        ...
        onEndKey(event, listType) {
            ...
            if (event.ctrlKey && event.shiftKey) {
                ...
                this.d_selection[listId] = [...this.value[listId]].slice(matchedOptionIndex, items.length);
                this.$emit('update:selection', this.d_selection);
                this.$emit('selection-change', {
                    originalEvent: event,
                    value: this.d_selection
