...
describe('PickList.vue', () => {
    ...
    it('should select an item from source list', async () => {
        await wrapper.vm.onItemClick({}, wrapper.vm.value[0][0], 0, 0);

        expect(wrapper.emitted()['update:selection'][0][0]).toEqual([[wrapper.vm.value[0][0]], []]);
    });

    it('should dblclick an item from source list', async () => {
        await wrapper.setProps({ selection: [[wrapper.vm.value[0][0]], []] });

        await wrapper.vm.onItemDblClick({}, wrapper.vm.value[0][0], 0);

        expect(wrapper.emitted()['input'][0][0][1]).toEqual([wrapper.vm.value[0][0]]);
        expect(wrapper.emitted()['move-to-target'][0]).toEqual([{ originalEvent: {}, items: [wrapper.vm.value[0][0]] }]);
        expect(wrapper.emitted()['update:selection'][0][0]).toEqual([[], []]);
    });

    it('should move item up', async () => {
        await wrapper.setProps({ selection: [[wrapper.vm.value[0][1]], []] });
