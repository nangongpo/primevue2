...
describe('OrderList.vue', () => {
    ...
    it('should select item', async () => {
        await wrapper.vm.onItemClick({}, wrapper.vm.value[0], 0);

        expect(wrapper.emitted()['update:selection'][0]).toEqual([[wrapper.vm.value[0]]]);

        await wrapper.setProps({ selection: [wrapper.vm.value[0]] });
    ...
    it('should change order', async () => {
        await wrapper.setProps({ selection: [wrapper.vm.value[2]] });
        await wrapper.setData({ d_selection: [wrapper.vm.value[2]] });

        expect(wrapper.findAll('li.p-orderlist-item')[2].classes()).toContain('p-highlight');

        await wrapper.vm.moveUp({});

        expect(wrapper.emitted()['input'][0][0][1]).toEqual(wrapper.vm.value[2]);
    });
});
