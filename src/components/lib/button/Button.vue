<template>
    <button v-ripple :class="cx('root')" type="button" :aria-label="defaultAriaLabel" :disabled="disabled" v-bind="getPTOptions('root')" v-on="$listeners" :data-p-severity="severity">
        <slot>
            <slot v-if="loading" name="loadingicon" :className="[cx('loadingIcon'), cx('icon')]">
            ...
            <slot v-else name="icon" :className="[cx('icon')]">
