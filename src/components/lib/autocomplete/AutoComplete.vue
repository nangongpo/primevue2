<template>
  <div
    ...
    <ul
      ...
      <li
        ...
        <slot name="chip" :value="option" v-bind="ptm('tokenLabel')">
          <span :class="cx('tokenLabel')" v-bind="ptm('tokenLabel')">
    ...
    <slot
      ...
      <i
        v-if="loadingIcon"
        :class="['pi-spin', cx('loadingIcon'), loadingIcon]"
        aria-hidden="true"
        v-bind="ptm('loadingIcon')" />
      <SpinnerIcon
        v-else
        :class="[cx('loadingIcon'), loadingIcon]"
        spin
        aria-hidden="true"
        v-bind="ptm('loadingIcon')" />
    </slot>
      ...
      <transition
        ...
        <div
          ...
          <slot
            name="header"
            :value="value"
            :suggestions="visibleOptions"></slot>
          <VirtualScroller
  ...
  methods: {
    ...
    isSelected(option) {
      ...
      return this.multiple
        ? (this.value || []).some((value) => this.isEquals(value, optionValue))
        : this.isEquals(this.value, this.getOptionValue(option))
    },
