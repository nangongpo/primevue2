        ...
        <Portal :appendTo="appendTo">
            <transition name="p-connected-overlay" @enter="onOverlayEnter" @after-enter="onOverlayAfterEnter" @leave="onOverlayLeave" @after-leave="onOverlayAfterLeave" v-bind="ptm('transition')">
                <div v-if="overlayVisible" :ref="overlayRef" :style="panelStyle" :class="[cx('panel'), panelClass]" @click="onOverlayClick" @keydown="onOverlayKeyDown" v-bind="{ ...panelProps, ...ptm('panel') }">
                    ...
                    <slot name="header" :value="value" :options="visibleOptions"></slot>
                    <div v-if="(showToggleAll && selectionLimit == null) || filter" :class="cx('header')" v-bind="ptm('header')">
                        <Checkbox
                            ...
                            <template #icon="slotProps">
                                <DynamicComponent v-if="$scopedSlots.headercheckboxicon" :template="$slots.headercheckboxicon" :checked="slotProps.checked" :className="slotProps.className" />
                                <DynamicComponent v-else-if="slotProps.checked" :template="checkboxIcon ? 'span' : 'CheckIcon'" :className="[slotProps.className, { [checkboxIcon]: slotProps.checked }]" v-bind="getHeaderCheckboxPTOptions('headerCheckbox.icon')" />
                            </template>
                        </Checkbox>
                        ...
                        <button v-ripple :class="cx('closeButton')" :aria-label="closeAriaLabel" @click="onCloseClick" type="button" v-bind="{ ...closeButtonProps, ...ptm('closeButton') }">
                            <slot name="closeicon" :className="cx('closeIcon')">
                                <DynamicComponent :template="closeIcon ? 'span' : 'TimesIcon'" :className="[cx('closeIcon'), closeIcon]" v-bind="ptm('closeIcon')" />
                            </slot>
                        </button>
                    </div>
                    <VirtualScroller 
                      :ref="virtualScrollerRef" 
                      v-bind="{ ...(virtualScrollerOptions || {}), ...ptm('wrapper') }" 
                                ...
                                <template v-for="(option, i) of items">
                                    ...
                                    <li
                                        ...
                                        <Checkbox :modelValue="isSelected(option)" :binary="true" :tabindex="-1" :variant="variant" :unstyled="unstyled" :pt="getCheckboxPTOptions(option, getItemOptions, i, 'itemCheckbox')">
                                            <template #icon="slotProps">
                                                <DynamicComponent v-if="$scopedSlots.itemcheckboxicon" :template="$slots.itemcheckboxicon" :checked="slotProps.checked" :className="slotProps.className" />
                                                <DynamicComponent
                                                    v-else-if="slotProps.checked"
                                                    :template="checkboxIcon ? 'span' : 'CheckIcon'"
                                                    :className="[slotProps.className, { [checkboxIcon]: slotProps.checked }]"
...
import SearchIcon from 'primevue2/icons/search';
...
import { ConnectedOverlayScrollHandler, DomHandler, ObjectUtils, UniqueComponentId, ZIndexUtils } from 'primevue2/utils';
...
export default {
    ...
    inheritAttrs: false,
    emits: ['input', 'change', 'focus', 'blur', 'before-show', 'before-hide', 'show', 'hide', 'filter', 'selectall-change'],
    ...
    methods: {
        ...
        onOptionSelect(event, option, index = -1, isFocus = false) {
            ...
            if (selected) value = this.value.filter((val) => !ObjectUtils.equals(val, this.getOptionValue(option), this.equalityKey));
        ...
        getSelectedItemsLabel() {
            ...
            if (pattern.test(selectedItemsLabel)) {
                return selectedItemsLabel.replace(selectedItemsLabel.match(pattern)[0], this.value.length + '');
            }
        ...
        removeOption(event, optionValue) {
            let value = this.value.filter((val) => !ObjectUtils.equals(val, optionValue, this.equalityKey));
        ...
        isSelected(option) {
            ...
            return (this.value || []).some((value) => this.isEquals(value, optionValue));
        },
    ...
    computed: {
        ...
        label() {
            ...
            if (this.value && this.value.length) {
                if (ObjectUtils.isNotEmpty(this.maxSelectedLabels) && this.value.length > this.maxSelectedLabels) {
        ...
        chipSelectedItems() {
            return ObjectUtils.isNotEmpty(this.maxSelectedLabels) && this.value && this.value.length > this.maxSelectedLabels ? this.value.slice(0, this.maxSelectedLabels) : this.value;
        },
        allSelected() {
        ...
        selectedMessageText() {
            return this.hasSelectedOption ? this.selectionMessageText.replaceAll('{0}', this.value.length) : this.emptySelectionMessageText;
        },
        focusedOptionId() {
