<template>
  <div>
    ...
    <p>
      DataTable requires a value as an array of objects and columns defined with Column component. Throughout the
      samples, a car interface having vin, brand, year and color properties is used to define an object to be displayed
      by the datatable. Cars are loaded by a CarService that connects to a server to fetch. Note that this is only for
      demo purposes, DataTable does not have any restrictions on how the data is provided.
    </p>
    ...
    <DocSectionCode :code="dynamicColumnsCode" />
    <DocSectionCode :code="dynamicColumnsCode2" importCode />
    ...
    <h5>Auto Layout</h5>
    <p>
      Default table-layout is fixed meaning the cell widths do not depend on their content. If you require cells to
      scale based on their contents set <i>autoLayout</i> property to true. Note that Scrollable and/or Resizable tables
      do not support auto layout due to technical limitations.
    </p>

    <h5>Templating</h5>
    <p>
      Field data of a corresponding row is displayed as the cell content by default, this can be customized using a
      <i>body</i> template where current row data and column properties are passed via the slot props. On the other
      hand, <i>header</i> and <i>footer</i> sections of a column can either be defined with the properties or the
      templates. Similarly DataTable itself also provides header and footer properties along with the templates for the
      main header and footer of the table.
    </p>
    <DocSectionCode :code="templatingCode" />

    <h5>Sizes</h5>
    <p>
      In addition to the regular table, a smal and a large version are available with different paddings. For a table
      with" smaller paddings use <i>p-datatable-sm</i> class and for a larger one use <i>p-datatable-lg</i>.
    </p>
    <DocSectionCode :code="sizesCode" />
    ...
    <DocSectionCode :code="columnGroupingCode" />

    <h5>Pagination</h5>
    <p>
      Pagination is enabled by setting <i>paginator</i> property to true and defining the <i>rows</i> property defines
      the number of rows per page. See the <router-link to="/paginator">Paginator</router-link> for the available
      customization options such as paginator templates, page links, rows per page options and more which can be passed
      through the DataTable.
    </p>
    <DocSectionCode :code="paginationCode" />

    <p>paginatorstart and paginatorend templates are available to specify custom content at the left and right side.</p>
    <DocSectionCode :code="paginationCode2" />

    <p>
      Paginator can also be programmed programmatically using a binding to the <i>first</i> property that defines the
      index of the first element to display. For example setting first to zero will reset the paginator to the very
      first page. This property also supports "sync" keyword in case you'd like your binding to be updated whenever the
      user changes the page.
    </p>
    <DocSectionCode :code="paginationCode3" />

    <h5>Sorting</h5>
    <p>
      Enabling <i>sortable</i> property at column component would be enough to make a column sortable. The property to
      use when sorting is the <i>field</i> by default and can be customized using the <i>sortField</i>.
    </p>
    <DocSectionCode :code="sortingCode" />

    <p>
      By default sorting is executed on the clicked column only. To enable multiple field sorting, set
      <i>sortMode</i> property to "multiple" and use metakey when clicking on another column.
    </p>
    <DocSectionCode :code="sortingCode2" />

    <p>
      In case you'd like to display the table as sorted per a single column by default on mount or programmatically
      apply sort, use <i>sortField</i> and <i>sortOrder</i> properties. These two properties also support the "sync"
      keyword to get updated when the user applies sort a column.
    </p>
    <DocSectionCode :code="sortingCode3" />

    <p>In multiple mode, use the <i>multiSortMeta</i> property and bind an array of SortMeta objects instead.</p>
    <DocSectionCode :code="sortingCode4" />
    <DocSectionCode :code="sortingCode5" importCode />

    <h5 id="filtering">Filtering</h5>
    <p>
      DataTable has advanced filtering capabilities that does the heavy lifting while providing flexible customization.
      Filtering has two layout alternatives defined with the <i>filterDisplay</i>. In <b>row</b> setting, filter
      elements are displayed in a separate row at the header section whereas in <i>menu</i> mode filter elements are
      displayed inside an overlay. Filter metadata is specified using the <i>filters</i> as a v-model and UI elements
      for the filtering are placed inside the filter template. The template filter gets a <i>filterModel</i> and
      <i>filterCallback</i>, use filterModel.value to populate the filter with your own form components and call the
      filterCallback with the event of your choice like @input, @change, @click.
    </p>
    <DocSectionCode :code="filteringCode" importCode />

    <h6>Filter Row</h6>
    <p>Input field is displayed in a separate header row.</p>
    <DocSectionCode :code="filterRowCode" />

    <h6>Filter Menu</h6>
    <p>Input field is displayed in an overlay.</p>
    <DocSectionCode :code="filterMenuCode" />

    <h6>Multiple Constraints</h6>
    <p>
      In "menu" display, it is possible to add more constraints to a same filter. In this case, metadata could be an
      array of constraints. The operator defines whether all or any of the constraints should match.
    </p>
    <DocSectionCode :code="multipleConstraintsCode" importCode />

    <h6>Populate Filters</h6>
    <p>
      Providing a filters with predefined values would be enough to display the table as filtered by default. This
      approach can also be used to clear filters progammatically.
    </p>
    <DocSectionCode :code="populateFiltersCode" importCode />

    <h6>Match Modes</h6>
    <p>
      Depending on the <i>dataType</i> of the column, suitable match modes are displayed. Default configuration is
      available at <i>PrimeVue.filterMatchModeOptions</i> which can be used to customize the modes globally for all
      tables.
    </p>
    <DocSectionCode :code="matchModesCode" importCode />

    <p>
      If you need to override the match modes for a particular column use the <i>filterMatchModeOptions</i> property and
      provide an array with label-value pairs.
    </p>
    <DocSectionCode :code="matchModesCode2" />
    <DocSectionCode :code="matchModesCode3" importCode />

    <h6>Custom Filter</h6>
    <p>
      Custom filtering is implemented using the <i>FilterService</i>, first register your filter and add it to your
      <i>filterMatchModeOptions</i>.
    </p>
    <DocSectionCode :code="customFilterCode" importCode />
    <DocSectionCode :code="customFilterCode2" importCode />

    <h6>Filter Slots</h6>
    <p>
      Filter menu overlay can be customized even further with various templates including <i>filterheader</i>,
      <i>filterfooter</i>, <i>filterclear</i>, <i>filterapply</i>. Example here changes the buttons and adds a footer.
    </p>
    <DocSectionCode :code="filterSlotsCode" />

    <h5>Selection</h5>
    <p>
      DataTable provides single and multiple selection modes on click of a row. Selected rows are bound to the
      <i>selection</i> property and updated using the v-model directive. Alternatively column based selection can be
      done using radio buttons or checkboxes using <i>selectionMode</i> of a particular column. In addition row-select
      and row-unselect events are provided as optional callbacks.
    </p>

    <p>
      The <i>dataKey</i> property identifies a unique value of a row in the dataset, it is not mandatory however being
      able to define it increases the performance of the table signifantly.
    </p>
    ...
    <p>
      ...
      toggled individually depending on the value of <i>metaKeySelection</i> property value which is true by default. On
      touch enabled devices metaKeySelection is turned off automatically. Additionally ShiftKey is supported for range
      selection.
    </p>
    <DocSectionCode :code="selectionCode2" />

    <p>
      If you prefer a radioButton or a checkbox instead of a row click, use the <i>selectionMode</i> of a column
      instead. Following datatable displays a checkbox at the first column of each row and automatically adds a header
      checkbox to toggle selection of all rows.
    </p>
    <DocSectionCode :code="selectionCode3" />

    <h5>Scrolling</h5>
    <p>
      DataTable supports both horizontal and vertical scrolling as well as frozen columns and rows. Scrollable DataTable
      is enabled using <i>scrollable</i> property and <i>scrollHeight</i> to define the viewport height.
    </p>
    <DocSectionCode :code="scrollingCode" />
    ...
    <ul>
      <li>
        Use <i>min-width</i> in vertical scrolling only so that when there is enough space columns may grow and for
        smaller screens a horizontal scrollbar is displayed to provide responsive design.
      </li>
      <li>When horizontal scrolling is enabled, prefer <i>width</i> instead of <i>min-width</i>.</li>
      <li>
        In vertical scrolling only, use <i>flex</i> to disable grow and shrink while defining a initial width. When
        horizontal scrolling is enabled, this is not required as columns do not grow or shrink in horizontal scrolling.
      </li>
    </ul>
    <DocSectionCode :code="columnWidthsOfAScrollableTableCode" />

    <h5>Flex Scroll</h5>
    <p>
      In cases where viewport should adjust itself according to the table parent's height instead of a fixed viewport
      height, set scrollHeight option as flex. In example below, table is inside a Dialog where viewport size
      dynamically responds to the dialog size changes such as maximizing.
    </p>
    <DocSectionCode :code="flexScrollCode" />

    <h5>Full Page Scroll</h5>
    <p>
      FlexScroll can also be used for cases where scrollable viewport should be responsive with respect to the window
      size. See the Full Page demo for an example.
    </p>
    <DocSectionCode :code="fullPageScrollCode" />

    <h6>Horizontal Scrolling</h6>
    <p>
      For horizontal scrolling, it is required to set <i>scrollDirection</i> to "horizontal" and give fixed widths to
      columns.
    </p>
    <DocSectionCode :code="horizontalScrollingCode" />
    ...
    <DocSectionCode :code="horizontalAndVerticalScrollingCode" />

    <h6>Frozen Rows</h6>
    <p>
      Frozen rows are used to fix certain rows while scrolling, this data is defined with the
      <i>frozenValue</i> property.
    </p>
    <DocSectionCode :code="frozenRowsCode" />

    <h6>Frozen Columns</h6>
    <p>
      Certain columns can be frozen by using the <i>frozen</i> property of the column component. In addition
      <i>alignFrozen</i> is available to define whether the column should be fixed on the left or right.
    </p>
    <DocSectionCode :code="frozenColumnsCode" />

    <h6>Scrollable RowGroup</h6>
    <p>
      Row groups with subheaders have exclusive support for filtering, when the table scrolls the subheaders stay fixed
      as long as their data are still displayed. No additional configuration is required to enable this feature. View
      the <router-link to="/datatable/rowgroup">Row Group</router-link> demo for an example.
    </p>

    <h5>Lazy Loading</h5>
    <p>
      Lazy mode is handy to deal with large datasets, instead of loading the entire data, small chunks of data is loaded
      by invoking corresponding callbacks such as paging and sorting. It is also important to assign the logical number
      of rows to totalRecords by doing a projection query for paginator configuration so that paginator displays the UI
      accordingly.
    </p>

    <p>
      Lazy loading is implemented by handling <i>page</i>, <i>sort</i>, <i>filter</i> events by making a remote query
      using the information passed to these events such as first offset, number of rows, sort field for ordering and
      filters. Note that, in lazy filtering totalRecords should also be updated to align the data with the paginator.
    </p>

    <p>
      Visit the <router-link to="/datatable/lazy">lazy loading</router-link> demo for an example with a remote
      datasource.
    </p>

    <h5>Row Expansion</h5>
    <p>
      Rows can be expanded to display additional content using the <i>expandedRows</i> property with the v-model
    ...
    <p>
      In cell editing provides a rapid and user friendly way to manipulate the data. The datatable provides a flexible
      API so that the editing behavior is implemented by the page author whether it utilizes v-model or vuex.
    </p>

    <p>
      Individual cell editing is configured by setting the <i>editMode</i> to <b>cell</b>, defining editors with the
      <b>editor</b> template along with the <i>@cell-edit-complete</i> event. The content of the editor defines how the
      editing is implemented. The editor template receives a clone of the row data and using
      <i>@cell-edit-complete</i> event the new value can be updated to the model or cancelled. This also provides
      flexibility to apply conditional logic such as implementing validations.
    </p>
    <DocSectionCode :code="inCellEditingCode" />
    <DocSectionCode :code="inCellEditingCode2" importCode />

    <p>
      Row Editing is specified by setting <i>cellEdit</i> as <b>row</b>, defining <i>editingRows</i> with the v-model
      directive to hold the reference of the editing rows, adding a row editor column to provide the editing controls
      and implementing <i>@row-edit-save</i> to update the original row data. Note that since <i>editingRows</i> is
      two-way binding enabled, you may use it to initially display one or more rows in editing more or programmatically
      toggle row editing.
    </p>
    <DocSectionCode :code="inCellEditingCode3" />
    <DocSectionCode :code="inCellEditingCode4" importCode />

    <h5>Column Resize</h5>
    <p>
      Columns can be resized using drag drop by setting the <i>resizableColumns</i> to true. There are two resize modes;
      "fit" and "expand". Fit is the default one and the overall table width does not change when a column is resized.
      In "expand" mode, table width also changes along with the column width. <i>column-resize-end</i> is a callback
      that passes the resized column header and delta change as a parameter.
    </p>
    <DocSectionCode :code="columnResizeCode" />

    <p>
      It is important to note that when you need to change column widths, since table width is 100%, giving fixed pixel
      widths does not work well as browsers scale them, instead give percentage widths.
    </p>
    <DocSectionCode :code="columnResizeCode2" />

    <h5>Column Reorder</h5>
    <p>
      Columns can be reordered using drag drop by setting the <i>reorderableColumns</i> to true.
      <i>column-reorder</i> is a callback that is invoked when a column is reordered. DataTable keeps the column order
      state internally using keys that identifies a column using the field property. If the column has no field, use
      columnKey instead as it is mandatory for columns to have unique keys when reordering is enabled.
    </p>
    <DocSectionCode :code="columnReorderCode" />

    <h5>Row Reorder</h5>
    <p>
      Data can be reordered using drag drop by adding a reorder column that will display an icon as a drag handle along
      with the <i>row-reorder</i> event which is <b>mandatory</b> to update the new order. Note that the reorder icon
      can be customized using <i>rowReorderIcon</i> of the column component.
    </p>
    <DocSectionCode :code="rowReorderCode" />
    <DocSectionCode :code="rowReorderCode2" importCode />

    <h5>Row Group</h5>
    <p>
      Row Grouping comes in two modes, in "subheader" mode rows are grouped by a header row along with an optional group
      footer. In addition, the groups can be made toggleable by enabling <i>expandableRowGroups</i> as true. On the
      other hand, the "rowspan" mode uses rowspans instead of a header to group rows. <i>groupRowsBy</i> property
      defines the field to use in row grouping. Multiple row grouping is available in "rowspan" mode by specifying the
      <i>groupRowsBy</i> as an array of fields.
    </p>

    <p>
      Example below demonstrates the all grouping alternatives. Note that data needs to be sorted for grouping which can
      also be done by the table itself by speficying the sort properties.
    </p>
    <DocSectionCode :code="rowGroupCode" />
    <DocSectionCode :code="rowGroupCode2" importCode />

    <h5>Data Export</h5>
    <p>DataTable can export its data in CSV format using <i>exportCSV()</i> method.</p>
    <DocSectionCode :code="dataExportCode" />
    <DocSectionCode :code="dataExportCode2" importCode />

    <h5>TableState</h5>
    <p>
      Stateful table allows keeping the state such as page, sort and filtering either at local storage or session
      storage so that when the page is visited again, table would render the data using its last settings. Enabling
      state is easy as defining a unique <i>stateKey</i>, the storage to keep the state is defined with the
      <i>stateStorage</i> property that accepts session for sessionStorage and local for localStorage. Currently
      following features are supported by TableState; paging, sorting, filtering, column resizing, column reordering,
      row expansion, row group expansion and row selection.
    </p>
    <DocSectionCode :code="tableStateCode" />
    <DocSectionCode :code="tableStateCode2" importCode />

    <h5>ContextMenu</h5>
    <p>
      DataTable provides exclusive integration with the ContextMenu component using, <i>contextMenu</i>,
      <i>contextMenuSelection</i> property along with the <i>row-contextmenu</i> event.
    </p>
    <DocSectionCode :code="contextMenuCode" />
    <DocSectionCode :code="contextMenuCode2" importCode />
    ...
    <h5>Loading</h5>
    <p>
      A loading status indicator can be displayed when the <i>loading</i> property is enabled. The icon is customized
      through <i>loadingIcon</i> property. Additionally an option loading template is available to render as the body
      until the data is loaded.
    </p>
    <DocSectionCode :code="loadingCode" />
    <DocSectionCode :code="loadingCode2" importCode />

    <h5>Responsive</h5>
    <p>
      DataTable responsive layout can be achieved in two ways; first approach is displaying a horizontal scrollbar for
      smaller screens and second one is defining a breakpoint to display the cells of a row as stacked. Scrollable
      tables use the scroll layout approach internally and do not require additional configuration.
    </p>

    <h6>Scroll Layout</h6>
    <p>
      Set <i>responsiveLayout</i> to scroll to enabled this layout. Note that, when scroll mode is enabled table-layout
      automatically switches to auto from fixed as a result table widths are likely to differ and resizable columns are
      not supported. Read more about
      <a href="https://www.w3schools.com/cssref/pr_tab_table-layout.asp">table-layout</a> for more details.
    </p>
    <DocSectionCode :code="scrollLayoutCode" />

    <h6>Stack Layout</h6>
    <p>In stack layout, columns are displayed as stacked after a certain breakpoint. Default is '960px'.</p>
    <DocSectionCode :code="stackLayoutCode" />

    <h5>Row and Cell Styling</h5>
    <p>
      Certain rows or cells can easily be styled based on conditions. Cell styling is implemented with templating
      whereas row styling utilizes the <i>rowClass</i> property which takes the row data as a parameter and returns the
      style class as a string.
    </p>
    <DocSectionCode :code="rowAndCellStylingCode" />
    <DocSectionCode :code="rowAndCellStylingCode2" importCode />
    <DocSectionCode :code="rowAndCellStylingCode3" importStyle />
      ...
      <table class="doc-table">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              Template of the paginator. See the <router-link to="/paginator">Paginator</router-link> for all available
              options.
            </td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td>session</td>
            <td>
              Defines where a stateful table keeps its state, valid values are "session" for sessionStorage and "local"
              for localStorage.
            </td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td>null</td>
            <td>
              A function that takes the row data as a parameter and returns a string to apply a particular class for the
              row.
            </td>
          </tr>
          <tr>
      ...
      <table class="doc-table">
        ...
        <tbody>
          <tr>
            ...
            <td>
              Callback to invoke on pagination. Sort and Filter information is also available for lazy loading
              implementation.
            </td>
          </tr>
          <tr>
...
import ColumnGroup from 'primevue2/columngroup';     //optional for column grouping
        `
      },
      baseCode: {
        basic: `
...
<DataTable :value="cars" :scrollable="true" scrollHeight="200px">
    ...
    <Column field="color" header="Color"></Column>
      ...
      columnWidthsOfAScrollableTableCode: {
        basic: `
...
<DataTable :value="customers" :scrollable="true"  scrollDirection="horizontal">
    ...
    <Column field="representative.name" header="Representative" footer="Representative" :style="{width:'200px'}"></Column>
      ...
      horizontalAndVerticalScrollingCode: {
        basic: `
