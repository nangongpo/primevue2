...
this.primevue2.datatable = (function (utils, api, Paginator, OverlayEventBus, Dropdown, Button, Ripple) {
  'use strict';
  ...
  var script$9 = {
    ...
    methods: {
        ...
        switch (event.key) {
          ...
          case 'ArrowDown':
            if (this.overlayVisible) {
              var focusable = utils.DomHandler.getFocusableElements(this.$refs.overlay);
              if (focusable) {
                focusable[0].focus();
              }
              event.preventDefault();
      ...
      findPrevItem: function findPrevItem(item) {
        ...
        if (prevItem) utils.DomHandler.hasClass(prevItem, 'p-column-filter-separator') ? this.findPrevItem(prevItem) : prevItem;else return item.parentElement.lastElementChild;
      },
      hide: function hide() {
        this.overlayVisible = false;
      },
      ...
      onOverlayEnter: function onOverlayEnter() {
        ...
        if (this.filterMenuStyle) {
          utils.DomHandler.applyStyle(this.$refs.overlay, this.filterMenuStyle);
        }
        this.$refs.overlay.style.zIndex = String(utils.DomHandler.generateZIndex());
        document.body.appendChild(this.$refs.overlay);
        utils.DomHandler.absolutePosition(this.$refs.overlay, this.$refs.icon);
    ...
    methods: {
      ...
      updateStickyPosition: function updateStickyPosition() {
        if (this.columnProp('frozen')) {
          ...
          if (align === 'right') {
            var right = 0;
            var next = utils.DomHandler.getNextElementSibling(this.$el, '.p-frozen-column');
            if (next) {
            ...
            var left = 0;
            var prev = utils.DomHandler.getPreviousElementSibling(this.$el, '.p-frozen-column');
            if (prev) {
  ...
  var script$4 = {
    ...
    updated: function updated() {
      ...
      if (this.d_editing && (this.editMode === 'cell' || this.editMode === 'row' && this.columnProp('rowEditor'))) {
        var focusableEl = utils.DomHandler.getFirstFocusableElement(this.$el);
    ...
    methods: {
      ...
      findPreviousEditableColumn: function findPreviousEditableColumn(cell) {
        ...
        if (prevCell) {
          if (utils.DomHandler.hasClass(prevCell, 'p-editable-column')) return prevCell;else return this.findPreviousEditableColumn(prevCell);
        } else {
          return null;
        }
      },
      findNextEditableColumn: function findNextEditableColumn(cell) {
        ...
        if (nextCell) {
          if (utils.DomHandler.hasClass(nextCell, 'p-editable-column')) return nextCell;else return this.findNextEditableColumn(nextCell);
        } else {
          return null;
        }
      },
  ...
  var script = {
    ...
    methods: {
      ...
      findNextSelectableRow: function findNextSelectableRow(row) {
        ...
        if (nextRow) {
          if (utils.DomHandler.hasClass(nextRow, 'p-selectable-row')) return nextRow;else return this.findNextSelectableRow(nextRow);
        } else {
          return null;
        }
      },
      findPrevSelectableRow: function findPrevSelectableRow(row) {
        ...
        if (prevRow) {
          if (utils.DomHandler.hasClass(prevRow, 'p-selectable-row')) return prevRow;else return this.findPrevSelectableRow(prevRow);
        } else {
          return null;
        }
      },
      ...
      resizeTableCells: function resizeTableCells(newColumnWidth, nextColumnWidth) {
        ...
        var widths = [];
        var headers = utils.DomHandler.find(this.$refs.table, '.p-datatable-thead > tr > th');
      ...
      restoreColumnWidths: function restoreColumnWidths() {
        ...
        if (this.columnWidthsState) {
          ...
          if (this.scrollable && widths && widths.length > 0) {
            ...
            utils.DomHandler.find(this.$refs.table, '.p-datatable-thead > tr > th').forEach(function (header, index) {
...
})(primevue2.utils, primevue2.api, primevue2.paginator, primevue2.overlayeventbus, primevue2.dropdown, primevue2.button, primevue2.ripple);
