...
this.primevue2.tabview = (function (utils, Ripple) {
  'use strict';
  ...
  var script = {
    ...
    methods: {
      ...
      getKey: function getKey(tab, index) {
        return tab.header ? utils.ObjectUtils.resolveFieldData(tab, tab.header) : index;
      },
      ...
      tabs: function tabs() {
        ...
        if (this.allChildren) {
          tabs = this.allChildren.filter(function (child) {
            return child.$vnode.tag.toLowerCase().indexOf('tabpanel') !== -1;
          });
        }
        return tabs;
      }
